...
public class UILaunch {
  ...
  public static String getAppDataDirectory() {
    if(IsWindows) {
      ...
      return Path.of(System.getProperty("user.home"), "Library", "Application Support").toString();
    } else {
      return Path.of(System.getProperty("user.home"), ".local", "share").toString();
    }
  }
  ...
  public static String getGDDirectLink(String id) {
    return String.format("https://drive.google.com/uc?export=download&id=%s", id);
  }

  // currently using UploadThing 
  public static final Map<String, String> UIL_FILE_URLS = new TreeMap<String, String>(
      Map.of(
          "2026inva", "https://jxjozsvaxe.ufs.sh/f/rp8xqAZwHWGMsQfev1SuUYmLZkPKCBW2jgO97yTxX3z8fG51",
          "2025state", "https://jxjozsvaxe.ufs.sh/f/rp8xqAZwHWGMgwpP5q7qQS3VsjYulG0hf5KBy71PCFkMpXOz",
          "2025district", "https://jxjozsvaxe.ufs.sh/f/rp8xqAZwHWGMpHR0juXhkE458vxVsmANXzC6oU7jiaP1Y2yq",
          "2025invb", "https://jxjozsvaxe.ufs.sh/f/rp8xqAZwHWGMWPQSg8taujbMxJ3Bnr7XY0vSelTwE4A25gqy",
          "2025inva", "https://jxjozsvaxe.ufs.sh/f/rp8xqAZwHWGMqPorvkwLlAd9j36NPomXBYGKuiSZqzkRrawt"));
  ...
  public static String downloadContestZip(String contestName) {
    ...
    try {
      ...
      ReadableByteChannel rbc = Channels.newChannel(url.toURL().openStream());
      Path outputPath = Path.of(DownloadDir, contestName + "Programming" + ".zip");
      FileOutputStream fileOut = new FileOutputStream(outputPath.toString());
  ...
  public static class Problem {
    ...
    public boolean judge(String studentCodePath) {
      ...
      boolean isCorrect = isCorrectOutput(executeStudentCode(true));
  ...
  public static class Competition {
    ...
    public void setup() {
      ...
      for (Problem p : problems.values()) {
        ...
        try {
          Files.createFile(
              Path.of(StudentSourceDir, String.format("%s.java", name)));
        } catch (IOException e) {
        }
      }
    }
    ...
    private String getStudentCodePath(Problem p) {
      return Path.of(
          StudentSourceDir,
          String.format("%s.java", p.name)).toString();
    }
    ...
    private void restart() {
      try {
        ...
        (new ProcessBuilder(
            "java",
            "UILaunch.java").inheritIO()).start().waitFor();
      } catch (Exception e) {
      }
      IOUtils.cleanUpTemp();
      System.exit(0);
    ...
    private void show(String problemName) {
      ...
      try {
        List<String> lines = Files.readAllLines(
            Path.of(StudentSourceDir, String.format("%s.java", p.getName())));
  ...
  private static String requestUserDataZip() {
    ...
    System.out.printf("Would you like to download one from the internet? (y/n) ");
    input = s.nextLine();
    if (input.equalsIgnoreCase("y")) {
      List<String> availCompetitions = UIL_FILE_URLS.keySet().stream().toList();
      System.out.println("Enter the contest number from the following list:");
      ...
      Path expectedPath = Path.of(DownloadDir, availCompetitions.get(choice) + "Programming.zip");
  ...
  private static String detectCompetitionZip(String[] args) {
    String shareDir = IOUtils.checkDir(
        CurrentDir,
        "UILCS",
        "Programming");
  ...
  public static void main(String[] args) {
    ...
    String competitionZip = detectCompetitionZip(args);

    if (competitionZip == null) {
      competitionZip = requestUserDataZip();
      if (competitionZip.isEmpty() ||
