...
public class UILaunch {
  ...
  // currently using UploadThing as file host due to UT Box being difficult to
  // download from
  ...
  public static class IOUtils {
    ...
    public static void unzipFallback(Path zipFilePath, Path destPath) {
      try {
        ProcessBuilder pb;
        if (IsWindows) {
          pb = new ProcessBuilder("expand-archive", "-LiteralPath", zipFilePath.toString(), "-DestinationPath",
              destPath.toString());
        } else {
          pb = new ProcessBuilder("unzip", zipFilePath.toString(), "-d", destPath.toString());
        }
        pb = pb.inheritIO();
        pb.start().waitFor();
      } catch (Exception e) {
    ...
    public static void unzip(String zipFilePath, String destDir) {
      ...
      try (
        ...
        System.out.println("Attempted to use system commands to unzip file...");
        unzipFallback(zipPath, destPath);
      }

      System.out.println("Unzipped file successfully. Searching for data.");
      ...
      if (F.size() >= 1) {
        CompetitionDir = F.stream().filter(f -> !f.contains("MACOS")).findFirst().orElse("");
